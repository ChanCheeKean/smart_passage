...
class MQTTClient(threading.Thread):
    ...
    def connect(self, address: str = "localhost", port: int = 1883, username: str = "user", password: str = "password")\
            -> None:
        try:
    ...
    def on_message(self, client, obj, msg):
        ...
        if msg.topic == "transcity/device/passage/obstacle_control/obstacle_state":
            ...
            if (data["obstacle_state"] == "OPENING_TO_ENTRY" or
                    data["obstacle_state"] == "OPENING_TO_EXIT" or
                    data["obstacle_state"] == "CLOSING_FROM_ENTRY" or
                    data["obstacle_state"] == "CLOSING_FROM_EXIT" or
                    data["obstacle_state"] == "STOPPED" or
                    data["obstacle_state"] == "BLOCKED" or
                    data["obstacle_state"] == "FORCED"):
                door_state = DoorStatus.MOVING
            elif data["obstacle_state"] == "OPENED_ENTRY":
                door_state = DoorStatus.OPENED_LEFT
            elif data["obstacle_state"] == "OPENED_EXIT":
                door_state = DoorStatus.OPENED_RIGHT
            elif data["obstacle_state"] == "CLOSED":
