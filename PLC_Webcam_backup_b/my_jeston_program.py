...
''' Ligne vertical pour le milieu de la gate qui sera la separation en 2 de la zone de safety '''
g_safety_middle = None
...
def load_gate_roi(file_name: str = "roi_gate.json") -> bool:
    ...
    Args:
        file_name (str): absolute path or file name of gate configuration json file

    Returns:
        bool: True if file is valid and loaded, False if file is not found

    """
    global g_gate_roi, g_safety_left, g_safety_middle, g_safety_right, g_threshold_height_foot
    try:
        with open(file_name, "r") as f:
            ...
            g_safety_middle = data["safety_middle"]
            g_safety_right = data["safety_right"]
            g_threshold_height_foot = data["threshold_foot"]
        return True
    except Exception as e:
        print("File not found or not valid => " + str(e))
        return False


def is_in_zone(zone_left, zone_right, line) -> bool:
    """Define if line is between zone_left and zone_right

    Args:
        zone_left: x coordinate of vertical line
        zone_right: x coordinate of vertical line
        line: x coordinate of vertical line

    Returns:
        bool: True if line is between zone_left, zone_right, False otherwise
...
def determine_presence_zone(blobs) -> None:
    """Update g_status depends on list of blob

    Depends on blob type (HUMAN or OBJECT) and in which zone it is.
    (see class Status)

    Args:
        blobs: List of blob detected in Gate

    """
    global g_gate_roi, g_safety_left, g_safety_right, g_safety_middle
    for blob in blobs:
        if blob.in_gate:
            if is_in_zone(blob.left, blob.right, g_safety_right) and is_in_zone(blob.left, blob.right, g_safety_left):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                    g_status.person_in_zone_safety_2 += 1
                    g_status.person_in_zone_2 += 1
                    g_status.person_in_zone_1 += 1
                elif blob.type == BlobType.OBJECT:
                    g_status.object_in_zone_safety_1 += 1
                    g_status.object_in_zone_safety_2 += 1
                    g_status.object_in_zone_2 += 1
                    g_status.object_in_zone_1 += 1
                blob.in_roi = True
                blob.zones = ["left", "safety_left", "safety_right", "right"]
            elif is_in_zone(blob.left, blob.right, g_safety_right) and is_in_zone(blob.left, blob.right, g_safety_middle):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                    g_status.person_in_zone_safety_2 += 1
                    g_status.person_in_zone_1 += 1
                if blob.type == BlobType.OBJECT:
                    ...
                    g_status.object_in_zone_1 += 1
                blob.in_roi = True
                blob.zones = ["safety_left", "safety_right", "right"]
            elif is_in_zone(blob.left, blob.right, g_safety_right) and not is_in_zone(blob.left, blob.right, g_safety_middle):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                    g_status.person_in_zone_1 += 1
                if blob.type == BlobType.OBJECT:
                    ...
                    g_status.object_in_zone_1 += 1
                blob.in_roi = True
                blob.zones = ["safety_right", "right"]
            elif is_in_zone(blob.left, blob.right, g_safety_left) and is_in_zone(blob.left, blob.right, g_safety_middle):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                    g_status.person_in_zone_safety_2 += 1
                    g_status.person_in_zone_2 += 1
                if blob.type == BlobType.OBJECT:
                    ...
                    g_status.object_in_zone_2 += 1
                blob.in_roi = True
                blob.zones = ["safety_left", "safety_right", "left"]
            elif is_in_zone(blob.left, blob.right, g_safety_left) and not is_in_zone(blob.left, blob.right, g_safety_middle):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_2 += 1
                    g_status.person_in_zone_2 += 1
                if blob.type == BlobType.OBJECT:
                    ...
                    g_status.object_in_zone_2 += 1
                blob.in_roi = True
                blob.zones = ["safety_left", "left"]
            elif is_in_zone(blob.left, blob.right, g_safety_middle):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                    g_status.person_in_zone_safety_2 += 1
                if blob.type == BlobType.OBJECT:
                    ...
                    g_status.object_in_zone_safety_2 += 1
                blob.in_roi = True
                blob.zones = ["safety_left", "safety_right"]
                ...
                blob.in_roi = True
                blob.zones = ["left"]
            elif is_in_zone(g_safety_left, g_safety_middle, blob.left) or is_in_zone(g_safety_left, g_safety_middle,
                                                                                    blob.right):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_2 += 1
                if blob.type == BlobType.OBJECT:
                    g_status.object_in_zone_safety_2 += 1
                blob.in_roi = True
                blob.zones = ["safety_left"]
            elif is_in_zone(g_safety_middle, g_safety_right, blob.left) or is_in_zone(g_safety_middle, g_safety_right, blob.right):
                if blob.type == BlobType.HUMAN:
                    g_status.person_in_zone_safety_1 += 1
                if blob.type == BlobType.OBJECT:
                    g_status.object_in_zone_safety_1 += 1
                blob.in_roi = True
                blob.zones = ["safety_right"]
...
def click_and_select_roi(event, x, y, flags, param):
    global g_threshold_height_foot, g_safety_left, g_safety_middle, g_safety_right, g_gate_roi, g_nb_click
    # Set entire gate ROI
...
def save_roi_file(json_file_name: str = "roi_gate.json") -> None:
    """Save g_gate_roi, g_safety_left, g_safety_middle, g_safety_right, g_threshold_height_foot in a json file

    Args:
        json_file_name: Absolute path or file name of json file to store roi configuration for next run of this script

    """
    global g_gate_roi, g_safety_left, g_safety_middle, g_safety_right, g_threshold_height_foot
    data = {"gate_box": g_gate_roi, "safety_left": g_safety_left, "safety_middle": g_safety_middle,
            "safety_right": g_safety_right, "threshold_foot": g_threshold_height_foot}
    with open(json_file_name, "w") as fileName:
        json.dump(data, fileName)


def set_roi(img: np.ndarray):
    """Set gate roi configuration

    Open a new window and ask user to click on image to define ROI of gate
...
def print_status_in_image(img: np.ndarray, width: int, height: int) -> None:
    ...
    zoneSafety1_presence_position = (int(width / 2), 50)
    zoneSafety1_person_position = (int(width / 2), 80)
    zoneSafety1_object_position = (int(width / 2), 110)
    color_presence = None
    if g_status.is_presence_zone_safety_1():
        color_presence = (0, 255, 0)
    else:
        color_presence = (255, 0, 0)
    cv2.putText(img, "Safety right", zoneSafety1_presence_position, cv2.FONT_HERSHEY_SIMPLEX, 1,
                color_presence, 3)
    cv2.putText(img, "Person = {}".format(g_status.person_in_zone_safety_1), zoneSafety1_person_position,
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
    cv2.putText(img, "Object = {}".format(g_status.object_in_zone_safety_1), zoneSafety1_object_position,
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)

    zoneSafety2_presence_position = (int(width / 2) - 200, 50)
    zoneSafety2_person_position = (int(width / 2) - 200, 80)
    zoneSafety2_object_position = (int(width / 2) - 200, 110)
    color_presence = None
    if g_status.is_presence_zone_safety_2():
        color_presence = (0, 255, 0)
    else:
        color_presence = (255, 0, 0)
    cv2.putText(img, "Safety left", zoneSafety2_presence_position, cv2.FONT_HERSHEY_SIMPLEX, 1,
                color_presence, 3)
    cv2.putText(img, "Person = {}".format(g_status.person_in_zone_safety_2), zoneSafety2_person_position,
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
    cv2.putText(img, "Object = {}".format(g_status.object_in_zone_safety_2), zoneSafety2_object_position,
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
    ...
    try:
       ...
       #  while display.IsOpen() and not g_stop_program:
        while not g_stop_program:
            ...
            # get door state
            doorStatus = DoorStatus.OPENED_LEFT
            ...
            for detection in detections:
                ...
                if detection.ClassID == 1:
                    if detection.Bottom > g_threshold_height_foot:
                        blob = Blob()
            ...
            cv2.line(frame, (g_safety_middle, 0), (g_safety_middle, height), (0, 255, 0), 3)
