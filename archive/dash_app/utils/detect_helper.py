...
def is_inside_gate(detection, gate_xyxy, in_safety=True):
    ...
    if in_safety:
        return (detection[0] < gate_xyxy['right']) and (detection[2] > gate_xyxy['left']) and (detection[3] > gate_xyxy['top']) and (detection[1] < gate_xyxy['bottom'])
    else:
        return (detection[3] > gate_xyxy['top']) and (detection[1] < gate_xyxy['bottom'])
...
def determine_human_presence(frame, detections, gate_xyxy, object_count, check_dir=True, paid_pos='left', id_store=[]):
    ...
    for det in detections:
        if (is_inside_gate(det, gate_xyxy, in_safety=False)):
            ...
            cv2.putText(
                frame, f'ID: {det[-1]}', (int(det[0]), int(det[1] + 30)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
...
def determine_object_presence(
    ...
    for i in range(0, len(contours)):
        ...
        area = (w * h) / 100
        if (w < 50) | (h < 50) | (size > 100) | ((y + h) < gate_xyxy['top']) | (y > gate_xyxy['bottom']):
            continue
...
def process_frame(
    ...
    try:
        ...
        # deepsort tracking
        ori_det, detections = track_id(frame, detections, deep_tracker)
        # print(detections)

        # detect non human object
        bgMask, object_count_list, oversize_flag = determine_object_presence(
            frame, ref_img, ori_det, gate_xyxy, object_count_list, bg_sub, obj_tracker, mask_human=True)

        # Render detection
        render(frame, gate_xyxy, trigger_distance, bench_start_loop)
        
        # zone count
        object_count_list, antidir_flag, id_store_list = determine_human_presence(frame, detections, gate_xyxy, object_count_list, True, id_store_list)
